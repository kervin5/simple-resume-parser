...
module.exports = {
  ...
  profiles: [
    ...
    [
      "linkedin.com",
      function (url, Resume, profilesWatcher) {
        download(`https://${url.replace(/\s/g, "")}`, function (data, err) {
          if (data) {
            var $ = cheerio.load(data),
              linkedData = {
                positions: {
  ...
  regular: {
    ...
    email: [/([a-z0-9_\.-]+)(\s*)@([\da-z\.-]+)\.([a-z\.]{2,6})/],
    // ^([+]?[\s0-9]+)?(\d{3}|[(]?[0-9]+[)])?([-]?[\s]?[0-9])+(\s*)
    phone: [
      /(?:Telephone:|Mobile:)(?:\s*)?([+(\[]?[0-9\s]+[)\]]?)?(\d{3}|[(]?[0-9]+[)])?([-]?[\s]?[0-9]+[\s]?[0-9]+)+(\s*)/im,
      /([+(\[]?[0-9\s]+[)\]]?)?(\d{3}|[(]?[0-9]+[)])?([-]?[\s]?[0-9]+[\s]?[0-9]+)+(\s*)/im,
    ],
  },
};
